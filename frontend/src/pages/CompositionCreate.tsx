...
const PLAYSTYLES = ['balanced', 'offensive', 'defensive', 'zerg', 'havoc'] as const;
...
export default function CompositionCreate() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h2 className="text-2xl font-bold tracking-tight text-white mb-6">Create Composition</h2>

      <form onSubmit={onSubmit} className="space-y-6 bg-slate-900/60 border border-purple-500/20 rounded-xl p-6">
        {/* Name */}
        <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-slate-300 mb-2">Squad Size</label>
            <Input
              type="number"
              min={1}
              max={50}
              value={squadSize}
              onChange={(e) => setSquadSize(parseInt(e.target.value || '0', 10))}
              className="bg-slate-800/50 border-slate-700 text-white"
            />
          </div>
          <div>
            <label className="block text-sm text-slate-300 mb-2">Playstyle</label>
            <select
              value={playstyle}
        ...
        {/* Professions */}
        <div>
          <label className="block text-sm text-slate-300 mb-2">Professions (comma-separated)</label>
          <Input
        ...
        <div className="flex items-center gap-3">
          <Button type="submit" className="bg-purple-600 hover:bg-purple-700">Create</Button>
          <Button type="button" variant="outline" onClick={() => navigate('/compositions')}>Cancel</Button>
        </div>
      </form>
    </div>
  );
