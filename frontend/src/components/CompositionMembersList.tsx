...
const getRoleIcon = (role: string) => {
  ...
  if (roleLower.includes("support") || roleLower.includes("boon"))
    return Sparkles;
...
const getRoleColor = (role: string) => {
  ...
  if (roleLower.includes("dps") || roleLower.includes("damage"))
    return "text-red-400 bg-red-500/20";
  if (roleLower.includes("tank")) return "text-blue-400 bg-blue-500/20";
  return "text-slate-400 bg-slate-500/20";
};

export default function CompositionMembersList({
  members,
  squadSize,
}: CompositionMembersListProps) {
  return (
      ...
      <CardContent>
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {members.map((member, index) => {
            ...
            const professionGradient =
              professionColors[member.profession_name] ||
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center space-x-3 mb-3">
                  <div
                    className={`w-12 h-12 rounded-full bg-gradient-to-br ${professionGradient} flex items-center justify-center text-white font-bold text-lg shadow-lg`}
                  >
                ...
                <div className="flex items-center space-x-2">
                  <div
                    className={`flex items-center space-x-1 px-2 py-1 rounded-md ${roleColorClass}`}
                  >
                ...
                {/* Notes */}
                {member.notes &&
                  member.notes !== member.role_type.replace("_", " ") && (
        ...
        {members.length < squadSize && (
          <div className="mt-4 p-3 rounded-lg bg-slate-800/30 border border-dashed border-slate-700">
            <p className="text-sm text-slate-400 text-center">
              {squadSize - members.length} slot
              {squadSize - members.length > 1 ? "s" : ""} disponible
              {squadSize - members.length > 1 ? "s" : ""}
            </p>
          </div>
        )}
      </CardContent>
