...
vi.mock("@/components/form/ProfessionSelect", () => ({
  ProfessionSelect: vi.fn(({ value = [], onChange }: { value: string[]; onChange: (value: string[]) => void }) => (
    <div data-testid="profession-select">
      ...
      <div data-testid="selected-professions">
        {value.map((prof: string) => (
          <div key={prof} data-testid={`selected-${prof}`}>{prof}</div>
        ))}
      </div>
    </div>
  ))
...
describe("CompositionForm", () => {
  ...
  const fillAndSubmitForm = async (data: {
    ...
    await userEvent.type(screen.getByLabelText(/nom de la composition/i), data.name)
  ...
  it("displays default values when provided", () => {
    ...
    expect(screen.getByDisplayValue(defaultValues.squad_size)).toBeInTheDocument()
