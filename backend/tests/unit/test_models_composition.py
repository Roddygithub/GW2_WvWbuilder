...
from app.models import Composition, User, Build, Role, Profession, CompositionTag
...
def sample_build(db, sample_user):
    ...
    build = Build(name="Test Build", game_mode="wvw", created_by_id=sample_user.id)
    db.add(build)
    db.commit()
    db.refresh(build)
    return build
...
def sample_role(db):
    ...
    role = Role(
        name="Test Role", description="Test Role Description", permission_level=1
    )
    db.add(role)
    db.commit()
    db.refresh(role)
...
def sample_profession(db):
    ...
    prof = Profession(name="Test Profession", game_modes=["wvw", "pve"])
    db.add(prof)
    db.commit()
    db.refresh(prof)
    return prof
...
def sample_composition(db, sample_user, sample_build):
    ...
    composition = Composition(
        **TEST_COMPOSITION_DATA, created_by=sample_user.id, build_id=sample_build.id
    )
    db.add(composition)
    db.commit()
...
class TestCompositionModel:
    ...
    def test_composition_required_fields(self, db, sample_user):
        ...
        with pytest.raises(IntegrityError):
            composition = Composition(description="No name", created_by=sample_user.id)
            db.add(composition)
            db.commit()
        db.rollback()
        ...
        with pytest.raises(IntegrityError):
            composition = Composition(name="No creator")
            db.add(composition)
            db.commit()
        db.rollback()

    def test_composition_relationships(
    ...
    def test_composition_tags(self, db, sample_composition):
        ...
        # Create a tag
        tag = CompositionTag(name="Test Tag", composition_id=sample_composition.id)
        db.add(tag)
        db.commit()
    ...
    def test_composition_validation(self, db, sample_user):
        ...
        with pytest.raises(ValueError):
            Composition(
                name="Invalid Squad Size", squad_size=0, created_by=sample_user.id
            )

        # Test long name
        with pytest.raises(ValueError):
            Composition(name="X" * 256, created_by=sample_user.id)  # Too long
...
class TestCompositionEdgeCases:
    ...
    def test_composition_tag_management(self, db, sample_composition):
        ...
        composition = add_composition_tag(
            db, composition_id=sample_composition.id, tag_name="Test Tag"
        )
