...
class TestIntegrationWithOptimizer:
    ...
    def test_mode_effects_applied_in_catalogue(self):
        ...
        herald_wvw = next(
            (
                t
                for t in engine_wvw.build_catalogue
                if t.profession_id == 2 and t.elite_spec_id == 5
            ),
            None,
        )
        herald_pve = next(
            (
                t
                for t in engine_pve.build_catalogue
                if t.profession_id == 2 and t.elite_spec_id == 5
            ),
            None,
        )
        ...
        if herald_wvw and herald_pve:
            ...
            assert herald_wvw.capabilities.get(
                "quickness", 0
            ) > herald_pve.capabilities.get("quickness", 0)
            assert herald_pve.capabilities.get(
                "alacrity", 0
            ) > herald_wvw.capabilities.get("alacrity", 0)
