...
async def test_update_user_with_existing_email(
    ...
    user2_in = UserCreate(
        email="another@example.com", password="testpassword", username="anotheruser"
    )
    ...
    with pytest.raises(Exception):  # Should raise IntegrityError
        await crud_user.update_async(
            db_session, db_obj=user2, obj_in=UserUpdate(email=test_user.email)
        )
...
async def test_update_user_with_invalid_data(db_session: AsyncSession, test_user: User):
    ...
    with pytest.raises(ValueError):
        await crud_user.update_async(
            db_session, db_obj=test_user, obj_in=UserUpdate(email="invalid-email")
        )
...
async def test_get_multi_with_pagination(db_session: AsyncSession, test_user: User):
    ...
    for i in range(1, 6):
        user_in = UserCreate(
            email=f"user{i}@example.com", password=f"password{i}", username=f"user{i}"
        )
        await crud_user.create_async(db_session, obj_in=user_in)
...
async def test_get_user_by_email_nonexistent(db_session: AsyncSession):
    ...
    user = await crud_user.get_by_email_async(
