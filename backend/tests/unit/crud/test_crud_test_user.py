...
async def test_create_user(db_session: AsyncSession):
    ...
    user_in = UserCreate(
        email="newuser@example.com", password="testpassword", username="newuser"
    )
...
async def test_get_user_by_email(db_session: AsyncSession, test_user: User):
    ...
    non_existent = await crud_user.get_by_email_async(
...
async def test_get_multi_users(db_session: AsyncSession, test_user: User):
    ...
    user3_in = UserCreate(
        email="user3@example.com", username="user3", password="testpass"
    )
    ...
    async with TestingSessionLocal() as empty_db_session:
        no_users = await crud_user.get_multi_async(
            empty_db_session, skip=1000, limit=10
        )
