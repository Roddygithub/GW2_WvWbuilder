...
@pytest.mark.asyncio
async def test_create_build(db_session: AsyncSession, test_user: User, test_profession: Profession):
    ...
    build = await crud_build.create_with_owner(db_session, obj_in=build_in, owner_id=test_user.id)
...
@pytest.mark.asyncio
async def test_get_multi_builds(db_session: AsyncSession, test_user: User, test_profession: Profession):
    ...
    for i in range(1, 6):
        ...
        await crud_build.create_with_owner(db_session, obj_in=build_in, owner_id=test_user.id)
...
@pytest.mark.asyncio
async def test_get_multi_by_owner(db_session: AsyncSession, test_user: User, test_profession: Profession):
    ...
    builds = await crud_build.get_multi_by_owner(db_session, owner_id=test_user.id, skip=0, limit=10)
...
async def test_update_build(db_session: AsyncSession, test_build: Build):
    ...
    updated_build = await crud_build.update(db_session, db_obj=test_build, obj_in=update_data)
...
@pytest.mark.asyncio
async def test_add_profession_to_build(db_session: AsyncSession, test_build: Build, test_profession: Profession):
    """Test adding a profession to a build."""
    # Add profession to build
    build = await crud_build.add_profession(db_session, build_id=test_build.id, profession_id=test_profession.id)
...
@pytest.mark.asyncio
async def test_remove_profession_from_build(db_session: AsyncSession, test_build: Build, test_profession: Profession):
    """Test removing a profession from a build."""
    # First, add a profession
    await crud_build.add_profession(db_session, build_id=test_build.id, profession_id=test_profession.id)

    # Now remove it
    build = await crud_build.remove_profession(db_session, build_id=test_build.id, profession_id=test_profession.id)

    # Verify the profession was removed
    assert len(build.professions) == 0

    # Test removing a non-existent profession (should be a no-op)
    build = await crud_build.remove_profession(db_session, build_id=test_build.id, profession_id=999)  # Non-existent ID
    assert build is not None
