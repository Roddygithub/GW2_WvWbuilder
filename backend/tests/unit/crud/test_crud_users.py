...
class TestCRUDUser:
    ...
    async def test_create_user_success(self, db: Session, user_data, mock_role):
        ...
        # Create the user
        result = user.create(db=mock_db, obj_in=UserCreate(**user_data))
    ...
    async def test_create_user_duplicate_email(self, db: Session, user_data, mock_user):
        ...
        with pytest.raises(ValidationException) as exc_info:
            user.create(db=mock_db, obj_in=UserCreate(**user_data))
    ...
    async def test_authenticate_success(self, db: Session, mock_user):
        ...
        # Test authentication
        result = user.authenticate(db=mock_db, email=TEST_EMAIL, password=TEST_PASSWORD)
    ...
    async def test_authenticate_wrong_password(self, db: Session, mock_user):
        ...
        result = user.authenticate(
            db=mock_db, email=TEST_EMAIL, password="wrongpassword"
        )
    ...
    async def test_update_user(self, db: Session, mock_user):
        ...
        # Perform the update
        result = user.update(
            db=mock_db, db_obj=mock_user, obj_in=UserUpdate(**update_data)
        )
    ...
    async def test_update_password(self, db: Session, mock_user):
        ...
        result = user.update_password(
            db=mock_db, db_user=mock_user, new_password=new_password
        )
