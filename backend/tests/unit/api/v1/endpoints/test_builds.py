...
class TestBuildsAPI:
    ...
    async def test_create_build_success(
        ...
        full_build_data = {
            ...
            "constraints": {"min_healers": 2, "min_dps": 5, "min_support": 2},
        }
    ...
    async def test_create_build_unauthorized(
        self, async_client: AsyncClient, test_profession: Dict[str, Any]
    ):
        ...
        response = await async_client.post(
            f"{settings.API_V1_STR}/builds/", json=build_data
        )
    ...
    async def test_update_build_success(
        ...
        update_data = {
            ...
            "is_public": False,
            "profession_ids": [
                p.id for p in self.professions[:2]
            ],  # Update with first two professions
        }
