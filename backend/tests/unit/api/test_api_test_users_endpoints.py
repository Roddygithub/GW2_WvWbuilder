...
def test_create_user(client: TestClient, db: Session) -> None:
    ...
    response = client.post(
        "/api/v1/users/", json=user_data, headers={"Authorization": f"Bearer {token}"}
    )
...
def test_get_user_by_id_superuser(client: TestClient, db: Session) -> None:
    ...
    regular_user = create_test_user(
        db, email="regular@example.com", username="regularuser"
    )

    # Create a superuser with is_superuser=True
    superuser = create_test_user(
        db, email="super@example.com", username="superuser", is_superuser=True
    )
