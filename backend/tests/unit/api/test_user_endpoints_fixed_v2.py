...
def override_get_current_active_user(current_user: User = Depends(override_get_current_user)):
    """Override for the get_current_active_user dependency for testing."""
...
def test_create_user_success(client: TestClient, db: Session) -> None:
    ...
    db.refresh(admin)  # Ensure we have the latest data
    assert admin.is_superuser is True, f"Admin user must be a superuser, but is_superuser={admin.is_superuser}"
...
def test_get_user_by_id_superuser(client: TestClient, db: Session) -> None:
    ...
    db.refresh(superuser)  # Ensure we have the latest data
