...
from app.models import Profession, EliteSpecialization, Build, User, Role, UserRole
...
async def test_get_elite_specs_by_game_mode(
    ...
    response = await async_client.get(
        "/api/v1/elite-specializations/by-game-mode/WVW"
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert len(data) == 2
    ...
    response = await async_client.get(
        "/api/v1/elite-specializations/by-game-mode/PVE"
    )
    data = response.json()
    assert len(data) == 1
