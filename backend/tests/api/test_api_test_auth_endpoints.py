...
async def test_change_password_success(
    client: AsyncClient, test_user, test_token, db_session
):
    """Test successfully changing the current user's password."""
    # Get current password hash
    result = await db_session.execute(
        "SELECT hashed_password FROM users WHERE id = :id", {"id": test_user.id}
    )
...
@pytest.mark.asyncio
async def test_change_password_incorrect_current(client: AsyncClient, test_token):
...
@pytest.mark.asyncio
async def test_logout(client: AsyncClient, test_token):
    """Test user logout."""
    # Make request to logout
    response = await client.post(
        "/api/v1/auth/logout", headers={"Authorization": f"Bearer {test_token}"}
    )
