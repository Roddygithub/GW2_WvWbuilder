...
class TestUserEndpoints:
    ...
    async def test_read_user_by_id_as_admin(
        ...
        response = await async_client.get(
            f"{settings.API_V1_STR}/users/{user_to_find.id}", headers=admin_headers
        )

        assert response.status_code == status.HTTP_200_OK
        assert response.json()["username"] == "find_me"

    async def test_read_nonexistent_user_as_admin(
        self, async_client: AsyncClient, auth_headers
    ):
        """Test that an admin cannot retrieve a non-existent user by ID."""
        admin_headers = await auth_headers(
            username="admin_reader_nonexistent", is_superuser=True
        )

        response = await async_client.get(
            f"{settings.API_V1_STR}/users/999999", headers=admin_headers
        )
        ...
        assert (
            ...
            or "non trouvé" in detail_lower
            or "trouvé" in detail_lower
        )
    ...
    async def test_read_user_by_id_forbidden_for_normal_user(
        ...
        response = await async_client.get(
            f"{settings.API_V1_STR}/users/{user_to_find.id}",
    ...
    async def test_admin_update_other_user(
        self, async_client: AsyncClient, user_factory, auth_headers
    ):
    ...
    async def test_admin_update_nonexistent_user(
        ...
        admin_headers = await auth_headers(
            username="admin_updater_nonexistent", is_superuser=True
        )
        update_data = {"is_active": False}
    ...
    async def test_read_users_as_admin(
        self, async_client: AsyncClient, user_factory, auth_headers
    ):
        ...
        response = await async_client.get(
            f"{settings.API_V1_STR}/users/", headers=admin_headers
        )
