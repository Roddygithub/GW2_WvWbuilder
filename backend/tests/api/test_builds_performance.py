...
class TestBuildsPerformance:
    ...
    async def test_update_build_performance(
        ...
        build = await build_factory(
            name="Performance Test Build to Update", is_public=True
        )
    ...
    async def test_large_payload_build_creation(
        ...
        large_description = (
            "a" * 1000
        )  # La description est limitée à 1000 caractères par le schéma
    ...
    async def test_memory_usage_on_listing(
        ...
        response = await async_client.get(
            f"{settings.API_V1_STR}/builds/", headers=headers
        )
    ...
    def _analyze_failures(self, responses):
        ...
        failures = [
            ...
            for r in responses
            if not (
                hasattr(r, "status_code") and r.status_code == status.HTTP_201_CREATED
            )
        ]
        if not failures:
            return "Aucun échec"
        ...
        for r in failures:
            error_msg = (
                ...
                else r.json().get("detail", str(r.status_code))
            )
        ...
        return "\n".join(
            f"- {count}x: {error}" for error, count in error_counts.items()
        )
    ...
    def _check_resource_usage(self, start_metrics, end_metrics, limits):
        ...
        if cpu_usage > limits["max_cpu_percent"]:
            raise AssertionError(
                f"CPU trop sollicité : {cpu_usage:.1f}% (limite: {limits['max_cpu_percent']:.1f}%)"
            )
    ...
    async def test_build_creation_under_load(
        ...
        print(
            f"Taux de réussite : {success_count}/{num_requests} ({success_rate:.2%}) en {duration:.4f}s"
        )
    ...
    async def test_user_journey_under_load(
        ...
        async def user_journey_task(i: int):
            try:
                ...
                headers = await auth_headers(
                    username=f"journey_user_{i}", password="password"
                )
                profession = await profession_factory()

                # 1. Création
                create_data = {
                    "name": f"Journey Build {i}",
                    "game_mode": "wvw",
                    "profession_ids": [profession.id],
                }
                ...
                read_res = await async_client.get(
                    f"{settings.API_V1_STR}/builds/{build_id}", headers=headers
                )
        ...
        success_count = sum(
            1
            for r in responses
            if hasattr(r, "status_code") and r.status_code == status.HTTP_200_OK
        )
        response_times = [
            r.elapsed.total_seconds() for r in responses if hasattr(r, "elapsed")
        ]
        ...
        if response_times:
            ...
            ), f"Le temps de réponse moyen ({avg_response_time:.4f}s) est trop élevé."
    ...
    async def test_rate_limiting_on_build_creation(
