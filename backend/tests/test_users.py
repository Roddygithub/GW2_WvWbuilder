...
class TestUserApi:
    ...
    async def normal_user_headers(
        client: TestClient, auth_headers: Callable, test_data: dict
    ) -> dict:
    ...
    async def superuser_headers(
        ...
        return await auth_headers(
            username=user.username, password="testpassword", is_superuser=True
        )

    async def test_get_users_as_superuser(
        self, client: TestClient, superuser_headers: dict, test_data: dict
    ):
        """Test that a superuser can retrieve a list of all users."""
        response = client.get(
            f"{settings.API_V1_STR}/users/", headers=superuser_headers
        )
    ...
    async def test_get_user_me(self, client: TestClient, normal_user_headers: dict):
        ...
        response = client.get(
            f"{settings.API_V1_STR}/users/me", headers=normal_user_headers
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    async def test_get_user_by_id_as_superuser(
        ...
        response = client.get(
            f"{settings.API_V1_STR}/users/{target_user_id}", headers=superuser_headers
        )
        assert response.status_code == status.HTTP_200_OK
        user_data = response.json()
    ...
    async def test_get_user_by_id_as_normal_user_forbidden(
        ...
        response = client.get(
            f"{settings.API_V1_STR}/users/{target_user_id}", headers=normal_user_headers
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    async def test_update_user_me(
        self, client: TestClient, normal_user_headers: dict, db: AsyncSession
    ):
    ...
    async def test_delete_user_as_superuser(
        self, client: TestClient, superuser_headers: dict, user_factory
    ):
        """Test that a superuser can delete a user."""
        # Create a user to be deleted
        user_to_delete = await user_factory(
            username="to_be_deleted", email="delete@me.com"
        )
