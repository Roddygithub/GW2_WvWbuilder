...
class TestCleanDB:
    ...
    async def test_clean_db_handles_foreign_keys(
        ...
        # This should work because clean_db handles foreign key constraints
        async with clean_db():
            # Verify all tables are empty
            result = await db.execute("SELECT COUNT(*) FROM roles")
            assert result.scalar_one() == 0, "Roles table should be empty after cleanup"

            result = await db.execute("SELECT COUNT(*) FROM permissions")
            assert (
                result.scalar_one() == 0
            ), "Permissions table should be empty after cleanup"
...
class TestDatabaseSetup:
    ...
    async def test_database_rollback_works(self, db: AsyncSession, clean_db):
        ...
        async with db.begin_nested():
            # Add test data
            user = User(email="rollback@test.com", hashed_password="password")
            db.add(user)
            await db.flush()
...
class TestFixturesIntegration:
    ...
    async def test_fixtures_work_together(
        ...
        async with clean_db():
            ...
            new_user = User(email="new@example.com", hashed_password="password")
            db.add(new_user)
            await db.commit()
