from typing import List, Optional, Dict, Any, Literal, ClassVar, Union
...
class BuildBase(BaseModel):
    ...
    game_mode: GameMode = Field(
        ...,
        description="Game mode this build is designed for",
        examples=["wvw"]
    )
...
class BuildProfessionBase(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, 
        description="Brief description of the profession"
    )
...
class BuildInDBBase(BaseModel):
        ...
        json_schema_extra={
            "examples": [{
                ...
                "professions": [
                    ...
                    {"id": 2, "name": "Firebrand", "description": "Guardian elite specialization"}
                ]
            }]
        }
    )
    ...
    updated_at: datetime = Field(..., description="Timestamp when the build was last updated")
    profession_ids: List[int] = Field(
        default_factory=list,
        ...
        if isinstance(data, dict):
            ...
            if 'professions' not in data:
                ...
                # Handle build_professions relationship if it exists
                    ...
                    } for bp in orm_obj.build_professions if hasattr(bp, 'profession') and bp.profession]
            ...
            # Handle case where build_professions is provided instead of professions
                ...
                data['professions'] = [{
                    ...
                    'name': bp.profession.name,
                    'description': bp.profession.description
                } for bp in data['build_professions'] if hasattr(bp, 'profession') and bp.profession]
...
class TeamMember(BaseModel):
    ...
    build: str = Field(..., description="Recommended build name", examples=["Firebrand - Quickness Support"])
...
class BuildGenerationResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message or error description", examples=["Build generated successfully"])
