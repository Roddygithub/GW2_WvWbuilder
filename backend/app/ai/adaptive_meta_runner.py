...
def run_adaptive_meta(with_llm: bool = False, dry_run: bool = False):
    ...
    logger.info(
        f"  → {len(all_changes)} total changes, {len(recent_changes)} in last 30 days"
    )
    ...
    if llm_engine:
        updated_synergies = recalculate_synergies(
            analyses, current_synergies, llm_engine
        )
    else:
    ...
    logger.info(
        f"Weights adjusted: {len([a for a in analyses if a['weight_delta'] != 0.0])}"
    )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Run without saving changes (test mode)"
    )
