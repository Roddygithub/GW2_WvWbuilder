...
def refresh_kb(with_llm: bool = False, auto: bool = False) -> None:
    ...
    if web_synergies:
        kb.meta["web_synergies"] = [{"pair": pair, "source": "web_crawl"} for pair in web_synergies]
    ...
    if with_llm and os.getenv("LLM_ENGINE") == "ollama":
        ...
        logger.info("Step 5/5: Skipping LLM enrichment (--with-llm not set or LLM_ENGINE != ollama)")
    ...
    if auto:
        logger.info("Auto-restart requested, but not implemented (manual restart required)")
        logger.info("  → Run: ./stop_all.sh && ./start_all.sh")


def main():
    parser = argparse.ArgumentParser(description="Refresh GW2_WvWbuilder Knowledge Base")
    parser.add_argument(
        "--with-llm",
        action="store_true",
